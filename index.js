...
var deepGetSet = require('deep-get-set');
...
module.exports = function (options) {
    ...
    function saveState(win) {
        ...
        try {
            if (config.object_path) {
                var _state = jsonfile.readFileSync(fullStoreFileName);
                // console.log(_state, config.object_path, state);
                deepGetSet(_state, config.object_path, state);
                // console.log(123, _state);
                
                mkdirp.sync(path.dirname(fullStoreFileName));
                jsonfile.writeFileSync(fullStoreFileName, _state);
            } else {
                mkdirp.sync(path.dirname(fullStoreFileName));
    ...
    try {
        ...
        if (config.object_path) {
            state = deepGetSet(_state, config.object_path);
        } else {
            state = _state;
        }
    } catch (err) {
